...
export default function HeroSection() {
    ...
    // Scroll progress over the full 280vh wrapper
    ...
    /* Reduced motion keeps the hand-off but drops the travel. The fade stays —
       it is the state change that tells you the hero is giving way to the
       signature. The scale and the 14px blur go, because those are the parts
       that read as the viewport moving underneath you. */
    const reduced = useReducedMotion();

    const contentOpacity = useTransform(scrollYProgress, [0, 0.32], [1, 0.05]);
    const contentScaleRaw = useTransform(scrollYProgress, [0, 0.32], [1, 0.86]);
    const blurAmount = useTransform(scrollYProgress, [0, 0.32], [0, 14]);
    const blurFilterRaw = useTransform(blurAmount, (v: number) => `blur(${v}px)`);
    const contentScale = reduced ? 1 : contentScaleRaw;
    const blurFilter = reduced ? 'none' : blurFilterRaw;

    /* Each signature sub-path draws independently; opacity snaps to kill dot
       artifacts. The trace itself is a fixed part of this site, so reduced
       motion does not remove it — it delivers the finished stroke instead of
       animating the pen, which keeps the moment without the scroll linkage. */
    ...
    const draw1 = reduced ? 1 : draw1Raw;
    const draw2 = reduced ? 1 : draw2Raw;
    const draw3 = reduced ? 1 : draw3Raw;
    const draw4 = reduced ? 1 : draw4Raw;

    const op1Raw = useTransform(scrollYProgress, [0, 0.02], [0, 1]);
    ...
    const op1 = reduced ? 1 : op1Raw;
    const op2 = reduced ? 1 : op2Raw;
    const op3 = reduced ? 1 : op3Raw;
    const op4 = reduced ? 1 : op4Raw;
            ...
            <div ref={wrapperRef} style={{ height: isMobile ? '180vh' : '280vh' }}>
                ...
                >
                    ...
                    <svg viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice"
                        ...
                        aria-hidden="true">
                        <defs>
                            <linearGradient id="hero-gc1" x1="0%" y1="0%" x2="100%" y2="100%">
                                <stop offset="0%" stopColor={ACCENT} stopOpacity="0" />
                                <stop offset="45%" stopColor={ACCENT} stopOpacity="0.10" />
                                <stop offset="100%" stopColor={ACCENT} stopOpacity="0" />
                            </linearGradient>
                            <linearGradient id="hero-gc2" x1="100%" y1="0%" x2="0%" y2="100%">
                                <stop offset="0%" stopColor={ACCENT} stopOpacity="0" />
                                <stop offset="50%" stopColor={ACCENT} stopOpacity="0.06" />
                                <stop offset="100%" stopColor={ACCENT} stopOpacity="0" />
                            </linearGradient>
                        </defs>
                        <path d="M -60,180 C 180,80 320,420 520,300 C 720,180 860,520 1100,440 C 1260,380 1380,200 1500,260"
                            fill="none" stroke="url(#hero-gc1)" strokeWidth="1.2" />
                        <path d="M -40,520 C 160,340 300,600 560,480 C 760,380 900,680 1200,580"
                            fill="none" stroke="url(#hero-gc2)" strokeWidth="1" />
                        <path d="M 800,0 C 1000,60 1180,200 1060,340 C 940,480 760,360 820,240"
                            fill="none" stroke="url(#hero-gc2)" strokeWidth="0.9" />
                    </svg>

                    {/* LEFT column — fades + blurs as signature draws in */}
                    <motion.div
                        style={{
                    ...
                    >
                        ...
                        {/* Animated gold rule — scaleX composites; animating width
                            reflows the whole column on every frame of the entrance. */}
                        <div style={{
                            width: 64, height: 1.5,
                            background: ACCENT, marginBottom: 36,
                            transformOrigin: 'left center',
                            transform: entered ? 'scaleX(1)' : 'scaleX(0)',
                            transition: 'transform 0.8s cubic-bezier(.22,1,.36,1) 0.2s',
                        }} />
                        ...
                        >
                            ...
                            {!isMobile ? (
                                <p style={{
                                    ...
                                    color: BODY_TEXT, lineHeight: 1.8, marginBottom: 0,
                                }}>
                                    {aboutText}
                                </p>
                            ) : (
                        ...
                        <div style={{
                            ...
                            {profile.socialLinks.map(s => (
                                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                                    style={{
                                        ...
                                        /* Standalone controls, not links inside a sentence, so they get a
                                           real 44px target, centred on the label. The underline lives on the
                                           inner span so it still hugs the text: put both on one element and
                                ...
                                >
                                    <span style={{ borderBottom: `1px solid ${ACCENT}33`, paddingBottom: 2 }}>
                                        {s.label}
                                    </span>
                                </a>
                            ))}
                            ...
                            <MagneticButton>
                                <a
                                    ...
                                    onMouseEnter={e => {
                                        (e.currentTarget as HTMLAnchorElement).style.background = ACCENT_DEEP;
                                        (e.currentTarget as HTMLAnchorElement).style.borderColor = ACCENT_DEEP;
                                    }}
                                    onMouseLeave={e => {
