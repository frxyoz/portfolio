...
// Signature path — split at ZM boundaries so each sub-path animates independently
...
export default function HeroSection() {
    ...
    // Hero content fades + scales back as signature draws in
    ...
    // Each signature sub-path draws independently; opacity snaps to kill dot artifacts
    const draw1 = useTransform(scrollYProgress, [0.05, 0.72], [0, 1]);
    const draw2 = useTransform(scrollYProgress, [0.10, 0.74], [0, 1]);
    const draw3 = useTransform(scrollYProgress, [0.07, 0.70], [0, 1]);
    const draw4 = useTransform(scrollYProgress, [0.40, 0.73], [0, 1]);
                    ...
                    >
                        ...
                        <div style={{
                            ...
                            <button
                                ...
                                onMouseEnter={e => {
                                    const dot = e.currentTarget.querySelector<HTMLElement>('.trig-dot');
                                    const label = e.currentTarget.querySelector<HTMLElement>('.trig-label');
                                    if (dot) dot.style.boxShadow = `0 0 0 8px ${ACCENT}38, 0 0 0 16px ${ACCENT}12`;
                                ...
                                onMouseLeave={e => {
                                    ...
                                    if (label) label.style.color = '#9a9088';
                                }}
                            >
                                <motion.div
                                    className="trig-dot"
                                    animate={{
                                        boxShadow: [
                                            `0 0 0 3px ${ACCENT}38, 0 0 0 7px ${ACCENT}12`,
                                            `0 0 0 7px ${ACCENT}28, 0 0 0 14px ${ACCENT}06`,
                                        ],
                                    }}
                                ...
                                <svg width="12" height="9" viewBox="0 0 12 9" fill="none"
                                    style={{ stroke: `${ACCENT}77`, marginTop: 1 }}>
                                    <polyline points="1,4.5 6,4.5 11,4.5" strokeWidth="1.2" strokeLinecap="round" />
                                    <polyline points="7.5,1 11,4.5 7.5,8" strokeWidth="1.2" strokeLinecap="round" />
                                </svg>
                            </button>
                        </div>
                        ...
                        <div style={{
                            ...
                            <span style={{ color: `${ACCENT}44`, fontSize: '0.5rem' }}>│</span>
