/* ── The marks themselves ──────────────────────────────────────────────────
   The authored graffiti, kept apart from both the wall that renders it and the
   corridor that places it, because both need it. `wall.tsx` needs the path data
   to draw a mark; `corridor.ts` needs the mark's size to guarantee that no two
   of them ever land on top of each other. One copy, so the drawing and the
   layout can never disagree about how big a piece is.
*/

/** Which tool made the mark, and so which family it is drawn from. */
export type MarkKind =
    | 'oz' | 'wildstyle' | 'throwup' | 'scrawl' | 'handstyle' | 'blockbuster' | 'chevron';
...
export const MARKS: Record<MarkKind, MarkArt[]> = {
