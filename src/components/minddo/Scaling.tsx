...
import { T, FONT, MONO, CONTROL, Arrowheads, Figure } from './ui';
...
export default function Scaling() {
    ...
    const controls = (
        <div style={{ display: 'flex', gap: 18, alignItems: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
                <label
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: FONT, fontSize: TYPE.COPY, color: T.body }}
                >
                    ...
                    <input
                        ...
                        className="minddo-range"
                        style={{ width: 220, minHeight: 30, padding: '4px 0' }}
                    />
                    <span aria-hidden style={{
                        fontFamily: FONT, fontSize: TYPE.BODY, fontWeight: 800, fontStretch: '104%',
                        color: T.ink, width: 28, ...TABULAR,
                    }}>{q}</span>
                </label>
    ...
    const panel = (
        <div style={{  fontFamily: FONT }}>
                <p style={{ fontFamily: FONT, fontSize: TYPE.COPY, color: T.body, lineHeight: 1.65 }}>
                    <strong style={{ color: T.ink, fontWeight: 700 }}>Why queue depth and not CPU.</strong>{' '}
                    ...
                    <strong style={{ color: T.ink, fontWeight: 700 }}>Why the ceiling is four.</strong>{' '}
    ...
    return (
        ...
        >
            <svg viewBox="0 0 1080 292"
                ...
                {/* Redis queue */}
                <rect x={14} y={30} width={250} height={216} fill={T.store.fill} stroke={T.store.stroke} strokeWidth={1.3} />
                ...
                {/* KEDA */}
                <rect x={308} y={54} width={266} height={168} fill={T.extern.fill} stroke={T.extern.stroke} strokeWidth={1.3} />
                ...
                {/* Worker pods */}
                <rect x={618} y={30} width={448} height={222} fill="none" stroke={T.rule} strokeWidth={1.4} strokeDasharray="4 4" />
                ...
                {Array.from({ length: MAX_R }).map((_, i) => {
                    ...
                    return (
                        <motion.g key={i} animate={{ opacity: live ? 1 : 0.17 }} transition={{ duration: 0.22 }}>
                            <rect x={x} y={y} width={202} height={60} fill={live ? T.tier.fill : T.canvas} stroke={live ? T.tier.stroke : T.rule} strokeWidth={1.2} />
                            ...
                            <rect
                                x={x + 92} y={y + 26} width={100} height={20}
