...
const MIN_R = 1, MAX_R = 8, PER_POD = 3, CONCURRENCY = 2;
...
export default function Scaling() {
    ...
    return (
        <Figure
            ...
            minWidth={900}
            caption="Drag the queue depth. The replica count is the real control law: one pod per three queued jobs, clamped to 1 through 8." controls={controls} panel={panel}
        >
            <svg viewBox="0 0 1080 292" style={{ width: '100%', display: 'block' }}>
                ...
                <text x={28} y={68} style={{ fontFamily: MONO, fontSize: 9, fill: T.store.text, opacity: 0.8 }}>LLEN celery = {q}</text>
                ...
                <text x={28} y={228} style={{ fontFamily: FONT, fontSize: 9.4, fill: T.store.text, opacity: 0.85 }}>
                    no AOF: a restart loses the queue
                </text>

                <path d="M 264 138 H 306" fill="none" stroke={T.ruleStrong} strokeWidth={1.3} markerEnd="url(#ah)" />
                <text x={268} y={130} style={{ fontFamily: FONT, fontSize: 9, fill: T.faint }}>poll 5 s</text>
                ...
                <text x={324} y={182} style={{ fontFamily: FONT, fontSize: 9.4, fill: T.extern.text, opacity: 0.85 }}>
                    managed HPA, 60 s cooldown
                </text>
                <text x={324} y={200} style={{ fontFamily: FONT, fontSize: 9.4, fill: q >= 24 ? T.bad.stroke : T.extern.text, opacity: 0.85 }}>
                    max 8 is a blast radius ceiling
                </text>

                <path d="M 574 138 H 616" fill="none" stroke={T.ruleStrong} strokeWidth={1.3} markerEnd="url(#ah)" />
                <text x={578} y={130} style={{ fontFamily: FONT, fontSize: 9, fill: T.faint }}>scale</text>
                ...
                <text x={634} y={68} style={{ fontFamily: MONO, fontSize: 9, fill: T.faint }}>
                    {pods} pod{pods === 1 ? '' : 's'} x concurrency {CONCURRENCY} = {slots} pipeline slots
                </text>

                {Array.from({ length: MAX_R }).map((_, i) => {
                    const live = i < pods;
                    const x = 634 + (i % 4) * 106;
                    const y = 84 + Math.floor(i / 4) * 76;
                    ...
                    return (
                        <motion.g key={i} animate={{ opacity: live ? 1 : 0.17 }} transition={{ duration: 0.22 }}>
                            <rect x={x} y={y} width={92} height={60} rx={5} fill={live ? T.tier.fill : '#fff'} stroke={live ? T.tier.stroke : T.rule} strokeWidth={1.2} />
                            <text x={x + 10} y={y + 19} style={{ fontFamily: MONO, fontSize: 9, fill: T.tier.text }}>worker-{i + 1}</text>
                            {[0, 1].map(s => (
                                <rect
                                    key={s}
                                    x={x + 10 + s * 38} y={y + 28} width={32} height={20} rx={3}
                                    fill={live && s < busy ? T.tier.stroke : '#fff'}
                ...
                <text x={634} y={236} style={{ fontFamily: FONT, fontSize: 9.4, fill: T.muted }}>
                ...
                <text x={452} y={272} style={{ fontFamily: FONT, fontSize: 9, fill: T.faint }}>workers pull with BRPOP</text>
            </svg>
        </Figure>
    );
}
